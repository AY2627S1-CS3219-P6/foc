...
Requester and courier are participation capabilities, not separate accounts or privileged system roles. `activeParticipationMode` is a profile preference used by the UI; it can be toggled between requester and courier but never grants or removes authorization. Order Service must still authorize each order action from the authenticated stable user ID and that order's participant relationship.
