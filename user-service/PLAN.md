...
| --- | --- | --- |
| `users` | `id` UUID PK; original and normalized unique `username`; unique normalized `email`; `display_name`; `system_role` enum (`USER`, `ADMIN`, `SUPER_ADMIN`); `account_status` enum (`ACTIVE`, `SUSPENDED`); `email_verified_at`; `active_participation_mode`; `role_version`; timestamps | Active identity and safe profile fields. Every public identifier is the immutable `id`. |
...
Indexes cover normalized username/email and the current Super Admin count. Later administrator search adds appropriately scoped indexes for user ID, username, display name, and email. There are no shared tables or foreign keys to another service.
...
| `DELETE /v1/users/me` | authenticated | Require current password and an explicit deletion acknowledgement; permanently remove credentials, sessions, and profile unless doing so would remove the last active Super Admin. |
...
- Implement the destructive account-deletion confirmation. Permanently remove the User Service's credential/profile/session data while retaining only de-identified external references and immutable audit records where legally/project-required.

**Human check:** the same account toggles requester/courier mode and retains its user ID; valid display-name changes persist. Attempts to alter email, username, role, account status, or ID fail and leave the profile unchanged. Deletion requires the warning acknowledgement and prevents further login.
...
- Enforce lifecycle invariants in a transaction with a lock: Admins cannot modify administrative roles; no user can self-change a system role; a request that would demote, suspend, delete, or revoke the last active Super Admin is rejected, including concurrent requests. A non-last Super Admin may delete their own account only after the normal password/acknowledgement confirmation.

**Human check:** bootstrap exactly one Super Admin without public registration. Promote a User to Admin, observe the audit record, then prove Admin cannot promote anyone. Attempt self-demotion and deletion of the sole Super Admin (both fail); add a second Super Admin and confirm the first can delete their own account. Run simultaneous last-Super-Admin demotion attempts and confirm all are denied.
...
| 5. Protected profiles | Allow-listed profile DTO, self-only endpoints, protected-field rejection, and deletion confirmation. |
