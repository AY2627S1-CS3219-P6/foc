...
| `M2F1.3.1`–`M2F1.3.3` | Remove confirmation calls `DELETE`, explains the current deactivation behavior, and refreshes the record. Inactive suppliers can be reactivated. | Permanent deletion of never-referenced suppliers and the Errand Service selection guard remain pending their service contract. |
| `M2F2.1.1`–`M2F2.1.4` | Normal list uses the active-only route; admin list uses the management route and includes status filtering. Both show required list fields and pagination/page-size controls. | API enforces visibility and paging. The UI deliberately requests six rows per page for the layout; the API default remains 20 when omitted. |
...
| `M2F3.1.1`–`M2F3.1.4` | Search submits `q` for name, area, or pickup text and trims whitespace. | API implements case-insensitive partial matching. |
| `M2F3.2.1`–`M2F3.2.4` | Category checkboxes send repeated `category` parameters; area text and admin-only status combine with search. | API implements category OR, cross-filter AND, exact normalized area, and active-only normal results. |
...
| `M2NFR3.4` | Removing or deactivating refreshes the admin view; normal browsing uses the active-only API. | Cross-client propagation within five seconds still needs a live integration test. |
...
removing/deactivating, searching, filtering, sorting, and paginating. Layouts
