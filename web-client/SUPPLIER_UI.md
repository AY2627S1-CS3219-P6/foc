...
| `M2F3.2.1`–`M2F3.2.4` | Category checkboxes send repeated `category` parameters; admin-only status combines with search. The separate exact-area field was removed from the UI. | API still supports the exact area filter, but backlog `M2F3.2.2` and the area portion of `M2F3.2.4` need revision if this simplified UI is the agreed product scope. |
